...
if [ "$MB_DEBUG" = "Y" ]; then
	echo "${Orange}We Got a Hit (iPad)-> $FoundItIOS${reset}"
fi
...
if [ "$MB_DEBUG" = "Y" ]; then
	echo "${Orange}We Got a Hit (Mac)-> $FoundItMACOS${reset}"
fi

####THIS IS WHERE WE SEARCH FOR IPADS
if [ ! -z "$FoundItIOS" ]; then
	if [ "$MB_DEBUG" = "Y" ]; then
		echo "${Orange}Tag Check-> $FoundItIOS${reset}"
	fi
...
if [ ! -z "$FoundItMACOS" ]; then
	if [ "$MB_DEBUG" = "Y" ]; then
		echo "${Orange}Tag Check-> $FoundItMACOS${reset}"
	fi
...
if [ -z "$FoundItIOS" ] && [ -z "$FoundItMACOS" ]; then
	cli_log "No hits for $1 in the Mac data.  Please double check your info and try again."
	exit 1
